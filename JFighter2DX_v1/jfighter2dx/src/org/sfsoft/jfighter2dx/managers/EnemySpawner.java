...
public class EnemySpawner {
	...
	public static Enemy createEnemy(EnemyType type, SpriteManager spriteManager) {
		...
		switch (type) {
			...
			case SMALL_ENEMY:
				enemy = new SmallEnemy(1000, new Random().nextInt(HEIGHT - ENEMY_HEIGHT), -50f);
				break;
			case SHOOTER_ENEMY:
				...
				if (location == 0) { 
					location = 20;
					enemy = new ShooterEnemy(1000, location, -120f, BulletDirection.DOWN);
				}
				else {
					location = HEIGHT - ENEMY_HEIGHT - 20;
					enemy = new ShooterEnemy(1000, location, -120f, BulletDirection.UP);
				}
				((ShooterEnemy) enemy).setEnemies(spriteManager.getEnemies());
				
				break;
			case STONE:
				enemy = new Stone(1000, new Random().nextInt(HEIGHT - ENEMY_HEIGHT), -50f);
				break;
			case BIG_ENEMY:
				enemy = new BigEnemy(1000, new Random().nextInt(HEIGHT - ENEMY_HEIGHT * 2), -100f);
				break;
			case PURSUER_ENEMY:
				enemy = new PursuerEnemy(1000, new Random().nextInt(HEIGHT - ENEMY_HEIGHT), -100f);
				((PursuerEnemy) enemy).setShip(spriteManager.getShip());
				break;
			case STATIC_SHOOTER_ENEMY:
				enemy = new StaticShooterEnemy(900, new Random().nextInt(HEIGHT - ENEMY_HEIGHT), -100f, BulletDirection.LEFT);
