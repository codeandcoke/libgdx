package com.arkabytes.jumper2dx.characters;

import com.arkabytes.jumper2dx.GameScreen;
import com.arkabytes.jumper2dx.Jumper2DX;
import com.arkabytes.jumper2dx.MainMenuScreen;
import com.arkabytes.jumper2dx.PauseScreen;
...
public class Player {
	...
	public Player(TiledMapTileLayer collisionLayer, Jumper2DX game) {
	...
	public void render(SpriteBatch spriteBatch) {
		...
		spriteBatch.draw(currentFrame, position.x, position.y, 18, 28);
	}
	...
	private void getTilesPosition(int startX, int startY, int endX, int endY, Array<Rectangle> tiles) {
		...
		for (int y = startY; y <= endY; y++) {
			for (int x = startX; x <= endX; x++) {
				...
				else if ((cell != null) && (cell.getTile().getProperties().containsKey("coin"))) {
					collisionLayer.setCell(xCell, yCell, collisionLayer.getCell(0, 5));
					game.coinSound.play();
					game.coins++;
				}
				// Si es un enemigo pierde una vida
				else if ((cell != null) && (cell.getTile().getProperties().containsKey("enemy"))) {
					if (!dead) {
						
						velocity.x = velocity.y = 0;
						game.lives--;
						game.levelSound.stop();
						if (game.lives == 0) {
				...
				// Si es caja de interrogante, TODO
				else if ((cell != null) && (cell.getTile().getProperties().containsKey("box"))) {
					
				}
				// Si es un cofre se abre y se termina la pantalla
				else if ((cell != null) && (cell.getTile().getProperties().containsKey("chest"))) {
					// TODO Terminar la pantalla y pasar a la siguiente
					if (!levelCleared) {
						levelCleared = true;
						game.levelSound.stop();
						game.levelClearSound.play();
						try {
							Thread.sleep(1000);
						} catch (InterruptedException ie) {}
